...
const Root = React.forwardRef<HTMLDivElement, OTPRootProps>(
	(
		...
		const [uncontrolledChars, setUncontrolledChars] = useState<string[]>(() =>
...
const Slot = React.forwardRef<HTMLInputElement, OTPSlotProps>(
	({ index, className, ...rest }, externalRef) => {
		const {
			chars,
			disabled,
			isComplete,
...
const Separator = React.forwardRef<HTMLSpanElement, OTPSeparatorProps>(
	({ children = '–', className, ...rest }, ref) => (
		<span ref={ref} className={className} aria-hidden="true" role="separator" {...rest}>
			{children}
		</span>
	),
)
