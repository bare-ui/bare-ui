...
function usePasswordContext() {
	...
	if (!ctx)
		throw new globalThis.Error('Password sub-components must be used within Password.Root')
	return ctx
}
...
const Root = React.forwardRef<PasswordHandle, PasswordRootProps>(
	(
		...
		const setError = useCallback(
			(hasError: boolean) => onErrorChange?.(hasError),
			[onErrorChange],
		)
		...
		return (
			<PasswordContext.Provider
				value={{
					...
					handleBlur,
					setFieldNode: (node) => {
						;(fieldRef as React.MutableRefObject<HTMLInputElement | null>).current =
							node
					},
				}}
			>
...
const Field = React.forwardRef<HTMLInputElement, PasswordFieldProps>(
	({ className, ...rest }, externalRef) => {
...
const Label = React.forwardRef<HTMLLabelElement, PasswordLabelProps>(
	({ children, className, ...rest }, ref) => {
		const { inputId, isRequired } = usePasswordContext()

		return (
			<label ref={ref} htmlFor={inputId} className={className} {...rest}>
...
const Error = React.forwardRef<HTMLElement, PasswordErrorProps>(
	({ children, className, ...rest }, ref) => {
