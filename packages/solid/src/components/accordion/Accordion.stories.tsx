...
const faqs = [
	{
		...
		answer: 'A headless component library. Zero styles shipped — you bring your own via class and data-attribute selectors.',
	},
	{
		value: 'item-2',
			...
			<For each={faqs}>
				{(faq) => (
					<Accordion.Item value={faq.value}>
						<Accordion.Trigger class={triggerCls}>
							{faq.question}
							<ChevronDown />
						</Accordion.Trigger>
						<Accordion.Content class='px-5 pb-4 text-sm leading-relaxed text-[#6b7280]'>
							{faq.answer}
						</Accordion.Content>
					</Accordion.Item>
				)}
	...
	render: () => (
		<Accordion.Root
			...
			<For each={richFaqs}>
				{(faq) => (
					<Accordion.Item value={faq.value}>
						<Accordion.Trigger class='flex w-full items-center gap-3 px-5 py-4 text-left text-sm font-medium text-black outline-none transition-colors hover:bg-[#f5f5f5] data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50'>
							<span class='text-xl'>{faq.icon}</span>
							<div class='flex-1'>
								<p class='text-sm font-medium text-black'>{faq.question}</p>
								<p class='text-xs text-[#6b7280] font-normal'>{faq.description}</p>
							</div>
							<ChevronDown />
						</Accordion.Trigger>
						<Accordion.Content class='px-5 pb-4 text-sm leading-relaxed text-[#6b7280]'>
							<p class='mb-2'>{faq.answer}</p>
							<button class='text-xs font-medium text-black hover:underline'>Learn more →</button>
						</Accordion.Content>
					</Accordion.Item>
				)}
			</For>
