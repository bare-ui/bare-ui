...
export interface CitationRootProps extends JSX.HTMLAttributes<HTMLDivElement> {
...
export interface CitationRefProps extends Omit<JSX.AnchorHTMLAttributes<HTMLAnchorElement>, 'children' | 'href'> {
	/** Id of the source this reference points to. */
	for: string;
	/** Static content or a render function. Defaults to a `<sup>` marker. */
	children?: JSX.Element | ((props: CitationRenderProps) => JSX.Element);
}

export interface CitationListProps extends Omit<JSX.OlHTMLAttributes<HTMLOListElement>, 'children'> {
	/** Render a single footnote. Defaults to a title + link row. */
	children?: (props: CitationRenderProps) => JSX.Element;
}
