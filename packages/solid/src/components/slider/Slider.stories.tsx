		...
		return (
			<div class='flex flex-col gap-3'>
				...
				<p class='text-sm text-[#6b7280]'>
					Value: <span class='font-medium text-black'>{value()}</span>
				</p>
			</div>
		);
	},
	...
	render: () => {
		...
		return (
			<div class='flex flex-col gap-3'>
				...
				/>
				<p class='text-sm text-[#6b7280]'>
					Range: <span class='font-medium text-black'>${range()[0]}</span> –{' '}
					<span class='font-medium text-black'>${range()[1]}</span>
				</p>
			</div>
		);
	},
...
export const Complex: Story = {
	render: () => {
		...
		const [temp, setTemp] = createSignal<[number, number]>([18, 24]);

		return (
			<div class='flex max-w-md flex-col gap-6 rounded-[20px] border border-black bg-white p-5'>
				<div>
					<div class='flex justify-between text-sm mb-2'>
						<span class='font-medium text-black'>Volume</span>
						<span class='text-[#6b7280]'>{volume()}%</span>
					</div>
					<Slider
						value={volume()}
						onChange={setVolume}
				...
				<div>
					<div class='flex justify-between text-sm mb-2'>
						<span class='font-medium text-black'>Brightness</span>
						<span class='text-[#6b7280]'>{brightness()}%</span>
					</div>
					<Slider
						value={brightness()}
				...
				<div>
					<div class='flex justify-between text-sm mb-2'>
						<span class='font-medium text-black'>Temperature</span>
				...
				<div>
					<p class='text-sm font-medium text-black mb-2'>Disabled</p>
					<Slider
						defaultValue={50}
						disabled
						aria-label='Disabled'
