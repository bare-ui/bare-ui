...
export const Composed: Story = {
	render: () => (
		<div class='flex flex-col gap-6'>
			<div>
				<p class='mb-2 text-xs font-semibold uppercase tracking-wide text-[#9ca3af]'>Unified</p>
				<Diff.Root
					oldValue={before}
					newValue={after}
					class={shellCls}>
			...
			<div>
				<p class='mb-2 text-xs font-semibold uppercase tracking-wide text-[#9ca3af]'>Split</p>
				<Diff.Root
					oldValue={before}
					newValue={after}
					class={shellCls}>
...
export function sub(a, b) {
  return a - b;
		...
		return (
			<div class='mx-auto w-full max-w-2xl overflow-hidden rounded-xl border border-[#e5e7eb] bg-white'>
				<Diff.Root
					oldValue={oldFile}
					newValue={newFile}>
					<div class='flex items-center justify-between border-b border-[#e5e7eb] bg-[#f9fafb] px-4 py-2'>
						<span class='font-mono text-xs text-[#374151]'>src/math.ts</span>
						<Diff.Stats>
							{({ additions, deletions }) => (
					...
					<Diff.Unified>
						{({ line }) => (
							<div class={`flex whitespace-pre px-1 font-mono text-xs ${lineClass(line.type)}`}>
								<span class={gutterCls}>{line.oldLine ?? ''}</span>
								<span class={gutterCls}>{line.newLine ?? ''}</span>
								<span class='px-2'>{sign(line.type)}</span>
								<span>{line.content}</span>
