...
export const Complex: Story = {
	render: () => {
		...
		return (
			<div class='w-[40rem] rounded-2xl border border-[#e5e7eb] bg-white p-5'>
				<div class='mb-4 flex items-center justify-between'>
					<h2 class='text-sm font-semibold text-black'>Performance</h2>
					<span class='text-xs text-[#9ca3af]'>Last 30 days</span>
				</div>
						...
						{(c) => (
							<Stat.Root class='rounded-xl border border-[#e5e7eb] p-4'>
								<div class='flex items-center justify-between'>
									<Stat.Label class='text-xs text-[#6b7280]'>{c.label}</Stat.Label>
									<Stat.Delta
										value={c.delta}
										class={deltaCls}>
								...
								<Stat.Value class='mt-1 block text-2xl font-semibold text-black'>{c.value}</Stat.Value>
								<Stat.Sparkline
									data={c.trend}
									width={180}
									height={32}
