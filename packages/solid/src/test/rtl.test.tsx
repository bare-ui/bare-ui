...
describe('RTL — TreeView', () => {
	const nodes: TreeNode[] = [{ id: 'src', label: 'src', children: [{ id: 'src/a', label: 'a' }] }];
	function renderTree() {
		return rtl(() => (
			<TreeView.Root nodes={nodes} renderItem={(node) => <span>{node.label as string}</span>} />
		));
	}
