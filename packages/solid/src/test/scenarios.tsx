/**
 * Shared SSR/hydration scenarios. This module is compiled twice:
 *   - by `vitest.ssr.config.ts` (`generate: 'ssr'`) to produce server markup, and
 *   - by `vitest.hydrate.config.ts` (`generate: 'dom', hydratable: true`) to hydrate it.
 *
 ...
 * determinism test instead, not here.
 */
...
export const scenarios: Record<string, () => JSX.Element> = {
	// --- Presentational (no 'use client') — must render with no client boundary.
	Badge: () => <Badge count={5} />,
	...
	// --- Generated ids (createId -> createUniqueId): must align server/client.
	Input: () => (
		<Input.Root>
	...
	// --- Portal-backed overlays, closed (portal renders nothing on the server).
	'Modal (closed)': () => (
		<Modal.Root>
			<Modal.Portal>
				<Modal.Overlay>
