	...
	render: () => (
		<div className='flex h-[500px] items-center justify-center bg-[#f3f4f6]'>
			<Sheet.Root snapPoints={[0.3, 0.6, 0.9]}>
				<Sheet.Trigger className='rounded-lg bg-black px-4 py-2 text-sm font-medium text-white'>
					Open sheet
				</Sheet.Trigger>
				<Sheet.Portal>
					<Sheet.Overlay className={overlayCls} />
					<Sheet.Content className={`${contentBaseCls} rounded-t-2xl`}>
						<Sheet.Handle className='mx-auto mt-3 h-1.5 w-10 shrink-0 cursor-grab rounded-full bg-[#d1d5db] active:cursor-grabbing' />
						<div className='overflow-auto p-5'>
							...
							<Sheet.Close className='mt-4 w-full rounded-lg border border-[#d1d5db] py-2 text-sm font-medium'>
								Done
							</Sheet.Close>
						</div>
					</Sheet.Content>
