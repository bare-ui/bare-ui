...
describe('Slider (single)', () => {
	it('renders a slider thumb with role and aria attributes', () => {
		render(<Slider defaultValue={20} min={0} max={100} step={1} aria-label='volume' />);
	...
	it('Home/End jump to min/max; PageUp/PageDown move 10x step', async () => {
		render(<Slider defaultValue={50} min={0} max={100} step={2} />);
		const thumb = screen.getByRole('slider');
		thumb.focus();
	...
	it('controlled: onChange fires but value sticks until parent updates', async () => {
		...
		render(<Slider value={10} min={0} max={100} step={1} onChange={onChange} />);
		const thumb = screen.getByRole('slider');
		thumb.focus();
		await userEvent.keyboard('{ArrowRight}');
	...
	it('disabled: keyboard does nothing', async () => {
		const onChange = vi.fn();
		render(<Slider defaultValue={5} disabled onChange={onChange} />);
		const thumb = screen.getByRole('slider');
		thumb.focus();
		await userEvent.keyboard('{ArrowRight}');
...
describe('Slider (range)', () => {
	it('renders two thumbs', () => {
		render(<Slider range defaultValue={[20, 80]} min={0} max={100} step={1} />);
	...
	it('keyboard moves the focused thumb only', async () => {
		const onChange = vi.fn();
		render(<Slider range defaultValue={[20, 80]} min={0} max={100} step={5} onChange={onChange} />);
		const thumbs = screen.getAllByRole('slider');
		thumbs[0].focus();
		await userEvent.keyboard('{ArrowRight}');
	...
	it('thumbs cannot cross — order is normalized', async () => {
		render(<Slider range defaultValue={[20, 30]} min={0} max={100} step={5} />);
		const thumbs = screen.getAllByRole('slider');
		thumbs[0].focus();
		// Push left thumb past right thumb
