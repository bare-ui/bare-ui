...
function renderFU(props: Partial<React.ComponentProps<typeof FileUpload.Root>> = {}) {
	return render(
			...
			<ul data-testid='files'>
				<FileUpload.Items>
					{(file, i, remove) => (
						<li key={i}>
							{file.name}
							<button aria-label={`remove ${file.name}`} onClick={remove}>×</button>
						</li>
					)}
				</FileUpload.Items>
			</ul>
...
describe('FileUpload', () => {
	...
	it('respects accept and calls onReject', async () => {
