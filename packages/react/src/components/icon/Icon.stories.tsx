...
export const Composed: Story = {
	render: () => (
		<div className='flex flex-col gap-8'>
			...
			<div>
				<p className='text-xs font-medium uppercase tracking-wide text-[#6b7280] mb-3'>Colors</p>
	...
	render: () => {
		...
		return (
			<div className='flex w-80 flex-col gap-4'>
				<div className='flex items-center gap-1 rounded-xl border border-[#e5e7eb] bg-white p-1.5'>
					{tools.map((t) => (
						<button
							key={t.type}
							type='button'
							aria-label={t.label}
							className='inline-flex size-9 items-center justify-center rounded-lg text-black hover:bg-[#f5f5f5]'>
							<Icon
								type={t.type}
								icons={icons}
								label={t.label}
				...
				<div className='flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 p-3'>
					...
					/>
					<div>
						<p className='text-sm font-medium text-amber-900'>Storage almost full</p>
						<p className='text-xs text-amber-700'>You’ve used 92% of your available space.</p>
					</div>
				</div>
			</div>
		);
