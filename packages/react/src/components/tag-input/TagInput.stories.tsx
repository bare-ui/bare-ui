...
export const Default: Story = {
	render: () => (
		<TagInput.Root defaultValue={['react', 'tailwind']} className={wrapperCls}>
			<TagInput.Items>
				{(tag, i, remove) => (
			...
			<TagInput.Field placeholder='Add a tag…' className={fieldCls} />
		</TagInput.Root>
	),
};
...
export const Composed: Story = {
	render: () => {
		const [tags, setTags] = useState<string[]>(['design', 'typography']);
		return (
			<div className='flex flex-col gap-2'>
				...
				<TagInput.Root value={tags} onChange={setTags} maxTags={5} className={wrapperCls}>
					...
					<TagInput.Field placeholder={tags.length >= 5 ? '' : 'Add up to 5 topics'} className={fieldCls} />
				</TagInput.Root>
	...
	render: () => {
		...
		return (
			<div className='flex max-w-md flex-col gap-2'>
				<label className='text-sm font-medium text-black'>Invite teammates</label>
				<TagInput.Root
					value={emails}
					onChange={setEmails}
					validate={validate}
					...
					<TagInput.Field placeholder='name@company.com' className={fieldCls} />
				</TagInput.Root>
