...
describe('isWireUiModuleSpecifier', () => {
	...
	it('ignores unrelated packages', () => {
		...
		expect(isWireUiModuleSpecifier('@radix-ui/react-accordion')).toBe(false)
...
describe('collectWireComponentsInFile', () => {
	it('finds known components imported from a Wire UI package', () => {
		...
		expect(collectWireComponentsInFile(ts, sf).sort()).toEqual(['Accordion', 'Switch'])
	})
...
describe('plugin factory', () => {
	it('exposes a create() that loads metadata and proxies the language service', () => {
		...
		const info = {
			languageService,
			project: { projectService: { logger: { info: (m: string) => logs.push(m) } } },
		}
		...
		expect(logs.some((l) => l.includes('no Wire UI components'))).toBe(true)

		// Passthrough: untouched members still resolve to the original behaviour.
		expect((proxy.getSemanticDiagnostics as () => unknown)()).toBe(passthroughSentinel)
	})
})
