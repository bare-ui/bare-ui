	...
	return render({
		template: `
			<ComboboxRoot :options="options" v-bind="rootProps">
				...
				<ComboboxContent>
					<ComboboxItems v-slot="{ option, highlighted, selected }">
						<div :data-state="highlighted ? 'highlighted' : selected ? 'selected' : 'idle'">
							{{ option.label }}
						</div>
					</ComboboxItems>
	...
	it('arrow navigation skips disabled options', async () => {
		...
		for (let i = 0; i < 6; i++) {
			...
			const highlighted = screen
				.getAllByRole('option')
				.find((o) => o.querySelector('[data-state="highlighted"]'));
			expect(highlighted?.textContent?.trim()).not.toBe('Solid');
		}
	});
