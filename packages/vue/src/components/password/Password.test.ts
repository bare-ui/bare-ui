...
function renderPassword(rootProps: Record<string, unknown> = {}) {
	return render({
		setup() {
			return () =>
				h(Password.Root, { ...rootProps }, () => [
					h(Password.Label, null, () => 'Password'),
					h(Password.Field, { placeholder: 'Enter password' }),
					h(Password.Toggle, null, () => 'Toggle'),
					h(Password.Error),
				]);
		},
	});
