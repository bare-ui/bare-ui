...
const tabbableDate = computed(() => {
	const inMonth = (d: Date | null | undefined) => (d && isSameMonth(d, ctx.month.value) ? startOfDay(d) : null);
...
function moveTo(target: Date) {
	...
	const monthDiff =
		(t.getFullYear() - ctx.month.value.getFullYear()) * 12 + (t.getMonth() - ctx.month.value.getMonth());
...
function handleKeyDown(e: KeyboardEvent) {
	...
	switch (e.key) {
		...
		case 'ArrowDown':
			...
			break;
		case 'Home':
			next = startOfWeek(base, ctx.weekStartsOn.value);
			break;
		case 'End':
			next = addDays(startOfWeek(base, ctx.weekStartsOn.value), 6);
			break;
		case 'PageUp':
			...
			<template
				...
				<slot
					...
					:weekday="{
						name: wd.name,
						short: wd.short,
						props: { role: 'columnheader', 'aria-label': wd.name },
					}">
					<div
						role="columnheader"
						:aria-label="wd.name"
