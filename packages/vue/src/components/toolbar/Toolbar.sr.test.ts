/**
 * Screen-reader semantics for Toolbar. Verifies the ARIA toolbar pattern a
 * screen reader navigates — role=toolbar, aria-orientation, toggle pressed
 * state, separator orientation, and roving-tabindex item accessibility.
 */
...
function renderToolbar(rootProps: Record<string, unknown> = {}) {
	return render({
		template: `
			<ToolbarRoot aria-label="Text formatting" v-bind="rootProps">
				<ToolbarToggle aria-label="Bold"><b>B</b></ToolbarToggle>
				<ToolbarToggle aria-label="Italic" :defaultPressed="true"><i>I</i></ToolbarToggle>
				<ToolbarSeparator />
				<ToolbarButton aria-label="Align left">&#8678;</ToolbarButton>
