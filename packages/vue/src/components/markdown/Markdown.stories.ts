...
const styled: MarkdownComponents = {
	...
	delete: (_props, { slots }) => h('del', { class: 'text-[#9ca3af]' }, slots.default?.()),
	link: (props, { slots }) =>
		h('a', { href: props.node.url, class: 'font-medium text-black underline' }, slots.default?.()),
	inlineCode: (props) =>
	...
	list: (props, { slots }) =>
		props.node.ordered ?
			h('ol', { class: 'mb-3 list-decimal space-y-1 pl-5 text-sm text-[#374151]' }, slots.default?.())
	...
	thematicBreak: () => h('hr', { class: 'mb-3 border-[#e5e7eb]' }),
};
...
const richDoc: MarkdownNode[] = [
	{ type: 'heading', depth: 3, children: [{ type: 'text', value: 'Supported nodes' }] },
	{
		...
		children: [
			...
			{ type: 'link', url: 'https://vuejs.org', children: [{ type: 'text', value: 'Vue' }] },
			{ type: 'text', value: ', ' },
...
export const Composed: Story = {
	render: () => ({
		setup: () => () =>
			h('div', { class: 'flex max-w-lg flex-col gap-8' }, [
				h('div', {}, [
					h(
						'p',
					...
					h(Markdown, { nodes: richDoc, components: styled }),
				]),
				h('div', {}, [
					h(
						'p',
...
const answer: MarkdownNode[] = [
	...
	},
	{
		...
		children: [
			...
			{
				type: 'listItem',
				children: [{ type: 'text', value: 'Keep the dependency array empty so the debounce is stable.' }],
			},
		],
	},
	{
...
export const Complex: Story = {
	render: () => ({
		setup: () => () =>
			h(
				'div',
				{ class: 'mx-auto max-w-xl rounded-2xl border border-[#e5e7eb] bg-white p-5 shadow-sm' },
				[
					...
					h(Markdown, { nodes: answer, components: styled }),
				],
			),
	}),
};
