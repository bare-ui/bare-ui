...
description: Install bareui and style your first component.
---
...
bareui has no production dependencies. React 19 is a peer dependency.
...
bareui ships no CSS. All interactive state is exposed through `data-*` attributes. Target them with any CSS approach you prefer.
...
You control the DOM order. Need the error above the field? Swap them. Need a wrapper div between label and field? Add it. bareui never dictates structure.

## Validation pattern

bareui components do **not** validate internally. You own validation — set `invalidType` when your logic determines the field is invalid:
