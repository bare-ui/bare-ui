...
description: Install bare-ui and style your first component.
---
...
bare-ui has no production dependencies. React 19 is a peer dependency.
...
bare-ui ships no CSS. All interactive state is exposed through `data-*` attributes. Target them with any CSS approach you prefer.
...
You control the DOM order. Need the error above the field? Swap them. Need a wrapper div between label and field? Add it. bare-ui never dictates structure.

## Validation pattern

bare-ui components do **not** validate internally. You own validation — set `invalidType` when your logic determines the field is invalid:
