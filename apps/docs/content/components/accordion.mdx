...
<template>
    ...
    <ForSolid>
    ```tsx
import { Accordion } from '@wire-ui/solid'
...
<template>
  <AccordionRoot
    ...
    class="w-full max-w-lg divide-y divide-black rounded-[20px] border border-black bg-white overflow-hidden"
  >
    <AccordionItem value="item-1">
...
>
  <Accordion.Item value="item-1">
    <Accordion.Trigger class="flex w-full items-center justify-between px-5 py-4 text-left text-sm font-medium text-black outline-none transition-colors hover:bg-[#f5f5f5] data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50">
      What is wire-ui?
      <svg
        class="size-4 shrink-0 text-black transition-transform duration-200 data-[state=open]:rotate-180"
        data-state="inherit"
