...
<template>
  <SearchRoot
    ...
    <ForSolid>
    ```tsx
import { Search } from '@wire-ui/solid'
...
<Search.Root
  ...
  <Search.Input
    ...
    class="w-full rounded-[8px] border border-black bg-white px-3 py-2 text-sm text-black outline-none focus:ring-2 focus:ring-black focus:ring-offset-1"
  />
  <Search.Content class="absolute left-0 top-full z-10 mt-1 w-full rounded-[20px] border border-black bg-white py-1">
    <For each={filtered()}>
      {(item) => (
        <Search.Item
          option={item}
