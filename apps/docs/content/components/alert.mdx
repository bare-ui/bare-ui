...
<Alert.Root status="warning">
  ...
  <Alert.Dismiss>
    ...
    ```vue
<script setup>
import { Alert, AlertDescription, AlertDismiss, AlertRoot, AlertTitle } from '@wire-ui/vue'
</script>
...
<template>
    ...
    <ForSolid>
    ```tsx
import { Alert } from '@wire-ui/solid'
...
```vue
<script setup>
import { AlertRoot, AlertTitle, AlertDescription } from '@wire-ui/vue'
</script>

<template>
  <AlertRoot status="warning" class="flex items-start gap-3 rounded-[8px] border border-black bg-[#f5f5f5] px-4 py-3 text-black">
    <div class="flex-1">
      <AlertTitle class="text-sm font-semibold">Warning</AlertTitle>
      <AlertDescription class="mt-0.5 text-sm">Your session is about to expire. Please save your work.</AlertDescription>
    </div>
    <button class="shrink-0 rounded-[8px] p-1 text-black hover:bg-[#e5e5e5] transition-colors">
      <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
      </svg>
    </button>
...
<Alert.Root status="warning" class="flex items-start gap-3 rounded-[8px] border border-black bg-[#f5f5f5] px-4 py-3 text-black">
  <div class="flex-1">
    <Alert.Title class="text-sm font-semibold">Warning</Alert.Title>
    <Alert.Description class="mt-0.5 text-sm">Your session is about to expire. Please save your work.</Alert.Description>
  </div>
...
Since `0.3.0`, the auto-dismiss timer is delegated to <ForReact>[`useTimeout`](../hooks/use-timeout)</ForReact><ForVue>[`useTimeout`](../hooks/use-timeout)</ForVue><ForSolid>[`createTimeout`](../hooks/use-timeout)</ForSolid> instead of inline `setTimeout` bookkeeping. No public API change.
