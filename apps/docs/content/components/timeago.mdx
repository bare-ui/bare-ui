...
<template>
    ...
    <ForSolid>
    ```tsx
import { Timeago } from '@wire-ui/solid'
...
<div>
  <span class="text-sm font-medium text-[#6b7280]">Duration: </span>
  <Timeago datetime={fiveMinutesAgo} isDuration class="text-sm text-black" />
</div>
<div>
  <span class="text-sm font-medium text-[#6b7280]">Full date/time: </span>
  <Timeago datetime={fiveMinutesAgo} class="text-sm text-black" />
...
<div>
  <span class="text-sm font-medium text-[#6b7280]">Time only: </span>
  <Timeago datetime={fiveMinutesAgo} timeOnly class="text-sm text-black" />
</div>
    ```
    </ForSolid>
  </Tabs.Tab>
...
In `isDuration` mode, the component recalculates the displayed string every minute via <ForReact>[`useInterval`](../hooks/use-interval)</ForReact><ForVue>[`useTimeout`](../hooks/use-timeout)</ForVue><ForSolid>[`createInterval`](../hooks/use-interval)</ForSolid>. The `datetime` attribute on the `<time>` element always holds the original ISO timestamp for machine readability.

## Internals

Since `0.3.0`, the live-update tick is delegated to the public timing <ForReact>hook</ForReact><ForVue>composable</ForVue><ForSolid>primitive</ForSolid> instead of an inline `setInterval`. No public API change.
