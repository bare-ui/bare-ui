...
The MCP server currently supports **React**. When Vue and Solid packages are released, the same `@wire-ui/mcp` server will serve documentation for all frameworks — just pass the `framework` parameter. No reinstallation needed.
