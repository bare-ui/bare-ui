...
|---|---|---|
| Shipped runtime (`@wire-ui/react`) | `react`, `react-dom` (peer) | MIT |
| Build/dev toolchain (full installed tree) | ~1,250 transitive | 100% permissive — MIT, ISC, Apache-2.0, BSD-2/3-Clause, BlueOak-1.0.0, 0BSD, Unlicense, CC0 |

A scan of the entire installed tree finds no copyleft license in any **distributed** code. The only non-permissive entry is `@img/sharp-libvips` (LGPL-3.0), a prebuilt native binary used solely for image optimization in the docs/Storybook build — it is a build-time tool and is never bundled into a published package. A handful of packages omit the SPDX `license` field in their metadata; their `LICENSE` files confirm they are MIT.
