...
`@wire-ui/react` ships with **zero runtime dependencies**. The only peer dependencies are React and React DOM. This minimizes the attack surface — there are no transitive dependencies that could introduce supply chain vulnerabilities.
...
Wire UI components never use `dangerouslySetInnerHTML` or insert raw HTML. All rendering uses React's standard JSX, which automatically escapes content and prevents XSS attacks.
