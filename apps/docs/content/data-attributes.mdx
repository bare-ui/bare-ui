...
description: Complete reference for all data-* attributes bareui exposes.
---

# Data Attributes

All interactive state in bareui is exposed through `data-*` attributes. An attribute is **present as an empty string** when active and **absent** when not — never `"true"` or `"false"`.
...
Set by the consumer directly as HTML attributes — bareui passes them through.
