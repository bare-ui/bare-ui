...
description: Complete reference for all data-* attributes bare-ui exposes.
---

# Data Attributes

All interactive state in bare-ui is exposed through `data-*` attributes. An attribute is **present as an empty string** when active and **absent** when not — never `"true"` or `"false"`.
...
Set by the consumer directly as HTML attributes — bare-ui passes them through.
